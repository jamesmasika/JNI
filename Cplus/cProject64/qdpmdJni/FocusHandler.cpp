...
void FocusMDHandler::OnFrontConnected()
{
	...
	printf("MdFocus,OnFrontConnected!");
...
void FocusMDHandler::OnFrontDisconnected() 
{
	...
	m_jniUtil.print(m_env, m_jniTemplate, "MsFocus:OnFrontDisconnected.\n");
}
...
int FocusMDHandler::reqCalcOneOption(JNIEnv *env, jobject reqMap, jint nRequestID)
{
	...
	jstring jstr;
	jclass cls = (env)->GetObjectClass(reqMap);
	m_jniUtil.print(env, m_jniTemplate, "进入req函数");
	...
	memset(&request, 0, sizeof(CFocusFtdcBasePricingParamField));
	strcpy(request.InstrumentID, InstrumentID);
	request.CalculateTarget = CalculateTarget;
	request.ExerciseType = ExerciseType;
	request.OptionsType = OptionsType;
	...
	request.OptionPrice = OptionPrice;
	request.Delta = Delta;
	m_jniUtil.print(env, m_jniTemplate, "退出req函数");
...
void FocusMDHandler::OnRspCalcOneOption(CFocusFtdcBasePricingParamField *pBasePricingParam, CFocusFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
	...
	FILE * fp;
	//fp = fopen("jsonlog.txt", "a+");
	//fprintf(fp, "%s\n", "进入rsp函数");
	m_jniUtil.print(m_env, m_jniTemplate, "进入rsp函数");
	if (pRspInfo == NULL) {
		m_jniUtil.print(m_env, m_jniTemplate, "pRspInfo is null");
		return;
	}
	...
	{
		//m_jniUtil.print(m_env, m_jniTemplate, "正在创建返回java对象");
	...
	//m_jniUtil.print(m_env, m_jniTemplate, "返回java对象成功");
	if (jmap != NULL) {
		m_env->DeleteLocalRef(jmap);
	}
	//fprintf(fp, "%s\n", "退出rsp函数");
	m_jniUtil.print(m_env, m_jniTemplate, "退出rsp函数");
	fclose(fp);
