...
#include "com_quantdo_focusmd_jni_FocusMDJniTemplate.h"
#include "JniUtil.h"
...
(JNIEnv *env, jclass jcls, jstring j_frontAddr, jstring j_userId, jstring j_password) {
	...
	JniUtil jniUtil;
	Global_jniTemplate = jniUtil.initClassHelper(env, "com/quantdo/focusmd/jni/FocusMDJniTemplate");
	...
	const char *brokerId = "1";//env->GetStringUTFChars(j_brokerId, NULL);
	int j_topicId = 1;
...
JNIEXPORT jint JNICALL Java_com_quantdo_focusmd_jni_FocusMDJniTemplate_reqCalcOneOption
(JNIEnv *env, jclass jcls, jobject obj, jint Reqid)
{
	JniUtil jniUtil;
	jniUtil.print(env, Global_jniTemplate, "reqCalcOneOption start>");
	jint result = -1;
	
	result = Global_mdClient->getFocusMDHandler()->reqCalcOneOption(env, obj, Reqid);
	...
	jniUtil.print(env, Global_jniTemplate, "CalcOneOption end>");
	return result;
}



JNIEXPORT jint JNICALL Java_com_quantdo_focusmd_jni_FocusMDJniTemplate_reqCalcBarrierOption
(JNIEnv *env, jclass jcls, jobject obj, jint Reqid)
{
	JniUtil jniUtil;
	jniUtil.print(env, Global_jniTemplate, "reqCalcBarrierOption start>");
	jint result = -1;

	result = Global_mdClient->getFocusMDHandler()->reqCalcBarrierOption(env, obj, Reqid);
...
JNIEXPORT jint JNICALL Java_com_quantdo_focusmd_jni_FocusMDJniTemplate_reqCalcCombinationOption
(JNIEnv *env, jclass jcls, jobject obj, jint Reqid)
{
	JniUtil jniUtil;
	jniUtil.print(env, Global_jniTemplate, "CalcOneOption start>");
	jint result = -1;
